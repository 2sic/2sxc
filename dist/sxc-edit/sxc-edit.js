...
angular.module("sxcFieldTemplates")
    .factory("dnnBridgeSvc", ["$modal", "$http", "promiseToastr", "eavConfig", "sxc", function ($modal, $http, promiseToastr, eavConfig, sxc) {
        var svc = {};
...
angular.module('SxcEditTemplates', []).run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('fields/hyperlink/hyperlink-default.html',
    ...
    "\n" +
    "ADAM - sponsored with ♥ by 2sic.com\"> <span class=input-group-btn style=\"vertical-align: top\"><button ng-if=\"to.settings['merged'].Buttons.indexOf('adam') > -1\" type=button class=\"btn btn-default icon-field-button\" ng-disabled=to.disabled tooltip=\"{{'Edit.Fields.Hyperlink.Default.AdamUploadLabel' | translate }}\" ng-click=vm.toggleAdam()><i class=eav-icon-apple></i></button> <button ng-if=\"to.settings['merged'].Buttons.indexOf('page') > -1\" type=button class=\"btn btn-default icon-field-button\" ng-disabled=to.disabled tooltip=\"{{'Edit.Fields.Hyperlink.Default.AdamUploadLabel' | translate }}\" ng-click=\"vm.openDialog('pagepicker')\"><i class=eav-icon-sitemap></i></button> <button ng-if=\"to.settings['merged'].Buttons.indexOf('more') > -1\" tabindex=-1 type=button class=\"btn btn-default dropdown-toggle icon-field-button\" dropdown-toggle ng-disabled=to.disabled><i class=eav-icon-options></i></button></span><ul class=\"dropdown-menu pull-right\" role=menu><li role=menuitem ng-if=\"to.settings['merged'].ShowAdam\"><a class=dropzone-adam ng-click=vm.toggleAdam() href=javascript:void(0);><i class=eav-icon-apple></i> <span translate=Edit.Fields.Hyperlink.Default.MenuAdam></span></a></li><li role=menuitem ng-if=\"to.settings['merged'].ShowPagePicker\"><a ng-click=\"vm.openDialog('pagepicker')\" href=javascript:void(0)><i class=eav-icon-sitemap xicon=home></i> <span translate=Edit.Fields.Hyperlink.Default.MenuPage></span></a></li><li role=menuitem ng-if=\"to.settings['merged'].ShowImageManager\"><a ng-click=\"vm.openDialog('imagemanager')\" href=javascript:void(0)><i class=eav-icon-file-image xicon=picture></i> <span translate=Edit.Fields.Hyperlink.Default.MenuImage></span></a></li><li role=menuitem ng-if=\"to.settings['merged'].ShowFileManager\"><a ng-click=\"vm.openDialog('documentmanager')\" href=javascript:void(0)><i class=eav-icon-file xicon=file></i> <span translate=Edit.Fields.Hyperlink.Default.MenuDocs></span></a></li></ul></div><div ng-if=vm.showPreview style=\"position: relative\"><div style=\"position: absolute; z-index: 100; background: white; top: 10px; text-align: center; left: 0; right: 0\"><img ng-src=\"{{vm.thumbnailUrl(2)}}\"></div></div><adam-hint class=field-hints></adam-hint><div ng-if=value.Value class=field-hints><a href={{vm.testLink}} target=_blank tabindex=-1 tooltip-html-unsafe={{vm.tooltipUrl(vm.testLink)}}><span>&nbsp;... {{vm.testLink.substr(vm.testLink.lastIndexOf(\"/\"), 100)}}</span></a></div></div></div><div><adam-browser content-type-name=to.header.ContentTypeName entity-guid=to.header.Guid field-name=options.key auto-load=false folder-depth=0 sub-folder=\"\" update-callback=vm.setValue register-self=vm.registerAdam ng-disabled=to.disabled></adam-browser><dropzone-upload-preview></dropzone-upload-preview></div></div>"
  );
  ...
  $templateCache.put('fields/string/string-font-icon-picker.html',
    "<div><div dropdown keyboard-nav auto-close=outsideClick is-open=vm.selectorIsOpen><div class=\"thumbnail-before-input icon-preview\"><button type=button tooltip={{value.Value}} dropdown-toggle><i class=\"{{vm.previewPrefix}} {{value.Value}}\" ng-show=value.Value></i> <span ng-show=!value.Value>&nbsp;&nbsp;</span></button></div><div class=input-group><input type=text class=\"form-control input-lg\" ng-model=value.Value ng-disabled=false dropdown-toggle></div><ul class=\"dropdown-menu icons-menu-columns\" role=menu><li class=input-group disable-auto-close><span class=\"input-group-addon btn-default btn\" ng-click=\"value.Value = ''\"><i class=eav-icon-cancel></i></span> <input type=search ng-model=vm.iconFilter class=\"makePaymentDropdownSearchBox form-control input-lg\" placeholder=\"search...\"></li><li ng-repeat=\"icn in vm.icons\" role=menuitem ng-show=\"icn.class.indexOf(vm.iconFilter) !== -1\"><a ng-click=vm.setIcon(icn.class) xng-click=\"value.Value = icn.class; status.isopen = false;\"><i class=\"{{vm.previewPrefix}} {{icn.class}}\"></i> <span tooltip={{icn.class}}>...{{icn.class.substring(vm.prefix.length-1,25)}}</span></a></li></ul></div><div ng-if=vm.debug.on>Infos: found {{vm.icons.length}} items for prefix \"{{vm.prefix}}\" and will use \"{{vm.previewPrefix}}\" as a preview class. Selected is \"{{value.Value}}\" and files are: {{vm.files}}</div></div>"
  );
