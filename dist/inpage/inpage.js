...
(function() {
    ...
    $2sxc._actions = {};
    $2sxc._actions.create = function(enableTools, enableDevelop, isContent, isDebug, contentTypeName) {
        var act = {
            //'test':     alert(createActionConfig),
            // todo: try to move all the action-definitions into a separate object and later into an own file
            // 1. move the stuff marked below to here
            // 2. ensure all dependencies are cleanly passed in, either in the constructor or in the calls
...
$2sxc.getManageController = function (id) {
    ...
    //    // in that case it's essential to add a contentType like 
    //    // <ul class="sc-menu" data-toolbar='{"action":"new", "contentType": "Category"}'></ul>
    //    'new': createActionConfig("new", "New", "plus", "default", false, { params: { mode: "new" },
    //        dialog: "edit", // don't use "new" (default) but use "edit"
    //        addCondition: function(settings, modConfig) {
    //            return modConfig.isList && settings.useModuleList && settings.sortOrder !== -1; // don't provide new on the header-item
    //        },
    ...
    //        }
    //    }),
    //    "metadata" : createActionConfig("metadata", "Metadata", "tag", "default", false, { params: { mode: "new" },
    //        dialog: "edit", // don't use "new" (default) but use "edit"
    //        dynamicClasses: function (settings) {
    //            // if it doesn't have data yet, make it less strong
    //            return settings.entityId ? "" : "empty";
    ...
    //    }
    //};
    var actionButtonsConf = $2sxc._actions.create(enableTools, enableDevelop, isContent, isDebug, contentTypeName);
