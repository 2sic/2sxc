...
public class AssemblyCacheManager(MemoryCacheService memoryCacheService) : ServiceBase(SxcLogName + ".AssCMn", connect: [memoryCacheService])
{
    ...
    private static string KeyAppCode(HotBuildSpec spec) => $"{GlobalCacheRoot}app:{spec.AppCacheKeyStable}.edition:{spec.Edition}.AppCode";
    ...
    private static string KeyDependency(HotBuildSpec spec) => $"{GlobalCacheRoot}app:{spec.AppCacheKeyStable}.edition:{spec.Edition}.dep:{DependenciesLoader.DependenciesFolder}";
