...
namespace ToSic.Sxc.Code
{
    ...
    public static class IDynamicCodeRoot16AsExtensions
    {
        ...
        private static ITypedEntity AsTyped(object target, DynamicEntity.MyServices services, int recursions, ILog log)
        {
            var l = log.Fn<ITypedEntity>();
            if (recursions <= 0)
                throw l.Ex(new Exception($"Conversion with {nameof(AsTyped)} failed, max recursions reached"));

            ITypedEntity convertOrNullAndLog(IEntity e, string typeName) => e == null
                ? l.ReturnNull($"empty {typeName}")
                : l.Return(new TypedEntity(e, services), typeName);
            switch (target)
            {
                case null:
        ...
        public static IEnumerable<ITypedEntity> AsTypedList(object list, DynamicEntity.MyServices services, int recursions, ILog log)
        {
            ...
            switch (list)
            {
                ...
                case ICanBeEntity _:
                    ...
                    return converted != null
                        ? l.Return(new List<ITypedEntity>() { converted }, "single item to list")
                        : l.Return(new List<ITypedEntity>(), "typed but converted to null; empty list");
