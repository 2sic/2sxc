...
internal class ModuleHtmlService() : ServiceBase(SxcLogName + ".ModSvc"), IModuleHtmlService
{
    ...
    /// </summary>
    private readonly Dictionary<int, ModuleHtml> _moduleTags = new();

    /// <inheritdoc />
    public IHtmlTag? AddTag(IHtmlTag tag, int moduleId, bool noDuplicates = false)
    {
    ...
    private ModuleHtml GetOrCreateModuleData(int moduleId)
    {
       if (_moduleTags.TryGetValue(moduleId, out var moduleServiceData))
