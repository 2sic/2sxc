...
public interface ICodeDataFactory: ICanGetService, IHasLog
{
    ...
    TCustom? AsCustomFrom<TCustom, TData>(TData? item, WrapDataSettings? settings = default)
        where TCustom : class, IDataWrapper;
    ...
    ITyped? AsTyped(object data, WrapDataSettings settings, string? detailsMessage = default);
    IEnumerable<ITyped>? AsTypedList(object list, WrapDataSettings settings);
    ...
    IDynamicEntity AsDynamic(IEntity entity, WrapDataSettings settings);

    /// <summary>
    /// Convert a list of Entities into a DynamicEntity.
    /// Only used in DynamicCodeRoot.
    /// </summary>
    IDynamicEntity AsDynamicFromEntities(IEnumerable<IEntity> list, WrapDataSettings settings, NoParamOrder npo = default, IEntity? parent = default, string? field = default);
    ...
    ITypedItem? AsItem(object? data, WrapDataSettings settings, NoParamOrder npo = default, ITypedItem? fallback = default);
    ...
    ITypedItem? AsItem(IEntity? entity, WrapDataSettings settings);

    IEnumerable<ITypedItem> EntitiesToItems(IEnumerable<IEntity> entities, WrapDataSettings settings);
    IEnumerable<ITypedItem> AsItems(object list, WrapDataSettings settings, NoParamOrder npo = default, IEnumerable<ITypedItem>? fallback = default);
    ...
    IField? Field(ITypedItem parent, bool supportOldMetadata, string? name, WrapDataSettings settings);
