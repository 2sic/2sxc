...
partial class PageService
{
    ...
    public string Activate(params string[] keys)
    {
        ...
        ExCtx?.GetState<IBlock>()?.BlockFeatureKeys.AddRange(added);
    ...
    private DynamicStack Settings => _settings.Get(() => _CodeApiSvc?.GetState<IDynamicStack>("Settings") as DynamicStack);
