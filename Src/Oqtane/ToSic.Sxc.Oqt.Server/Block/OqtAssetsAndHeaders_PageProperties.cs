            ...
            foreach (var p in props)
                switch (p.Property)
                {
                    ...
                    case PageProperties.Title:
                        result.Add(new OqtPagePropertyChanges { Property = OqtPageProperties.Title, Value = p.Value, Placeholder = p.ReplacementIdentifier, Change = GetOp(p) });
                        break;
                    case PageProperties.Description:
                        result.Add(new OqtPagePropertyChanges { Property = OqtPageProperties.Description, Value = p.Value, Placeholder = p.ReplacementIdentifier, Change = GetOp(p) });
                        break;
                    case PageProperties.Keywords:
                        result.Add(new OqtPagePropertyChanges { Property = OqtPageProperties.Keywords, Value = p.Value, Placeholder = p.ReplacementIdentifier, Change = GetOp(p) });
                        break;
                    default: // ignore
                        break;
                }
