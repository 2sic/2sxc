...
public class HtmlHelper
{
    ...
    {
        if (html == null || name.IsNullOrEmpty())
            return html;
        var pattern = "(<meta\\s+name\\s*=\\s*[\"']" + WebUtility.HtmlEncode(name) + "[\"']\\s+content\\s*=\\s*[\"'])(.*?)([\"']\\s*/?>)";
        ...
        var interpolatedStringHandler = new DefaultInterpolatedStringHandler(25, 3);
        interpolatedStringHandler.AppendLiteral("<meta name=\"");
        interpolatedStringHandler.AppendFormatted(WebUtility.HtmlEncode(name));
        interpolatedStringHandler.AppendLiteral("\" content=\"");
        interpolatedStringHandler.AppendFormatted(encode ? WebUtility.HtmlEncode(content) : content);
