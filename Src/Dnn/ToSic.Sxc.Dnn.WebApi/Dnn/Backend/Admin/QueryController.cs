    ...
    firstMessage: $"Query: {HttpContext.Current?.Request.Url.AbsoluteUri.After("/query/")}"), IQueryController
{
    ...
    [HttpGet] public QueryDefinitionDto Get(int appId, int? id = null)
        => Real.Get(appId, id);

    [HttpGet] public IEnumerable<DataSourceDto> DataSources(int zoneId, int appId)
    ...
    [HttpGet] public QueryRunDto Run(int appId, int id, int top = 0)
    ...
    [HttpGet] public void Clone(int appId, int id)
        => Real.Clone(appId, id);


    [HttpDelete] public bool Delete(int appId, int id)
        => Real.DeleteIfUnused(appId, id);

    [HttpPost] public bool Import(EntityImportDto args)
        => Real.Import(args);
}