    ...
    public abstract class SexyControlEditBase : PortalModuleBase
    {
        ...
        protected List<Element> Elements
        {
            get
            {
                if (_Elements == null)
                {
                    _Elements = Sexy.GetContentElements(ModuleId, Sexy.GetCurrentLanguageName(), null, PortalId, SexyContent.HasEditPermission(this.ModuleConfiguration), UserMayEditThisModule).ToList();
                }
                return _Elements;
            }
        }
