...
namespace ToSic.SexyContent
{
    public class Installer
    {
        ...
        public static readonly string[] UpgradeVersionList =
        {
            ...
            "08.03.00", "08.03.01", "08.03.02", "08.03.03", "08.03.04", "08.03.05", "08.03.06", "08.03.07"
        };
        ...
        private const string LogDirectory = "~/DesktopModules/ToSIC_SexyContent/Upgrade/Log/";
        ...
        {
            LogUpgradeStep(version, "UpgradeModule starting", false);
            ...
            LogUpgradeStep(version, "----- Upgrade to " + version + " started -----");
            ...
            try
            {
                ...
                LogUpgradeStep(version, "----- Upgrade to " + version + " completed -----");
        ...
        internal static void FinishAbortedUpgrade()
        {
            ...
            LogUpgradeStep("", "FinishAbortedUpgrade done", false);
        ...
        {
            if (!Directory.Exists(HostingEnvironment.MapPath(LogDirectory)))
                Directory.CreateDirectory(HostingEnvironment.MapPath(LogDirectory));

            var logFilePath = HostingEnvironment.MapPath(LogDirectory + version + ".resources");
        ...
        internal static void SaveDetailedLog()
        {
            if (!Directory.Exists(HostingEnvironment.MapPath(LogDirectory)))
                Directory.CreateDirectory(HostingEnvironment.MapPath(LogDirectory));
        ...
        {
            LogUpgradeStep(version, "IsUgradeComplete checking", false);
            var logFilePath = HostingEnvironment.MapPath(LogDirectory + version + ".resources");
            var complete= File.Exists(logFilePath);
            LogUpgradeStep(version, "IsUgradeComplete: " + complete, false);
            return complete;
        }
        ...
        internal static bool IsUpgradeRunning
        {
            get
            {
                ...
                var lockFilePath = HostingEnvironment.MapPath(LogDirectory + "lock.resources");
            ...
            private set
            {
                ...
                else
                {
                    ...
                    var renamedLockFilePath =
                        HostingEnvironment.MapPath(LogDirectory + DateTime.UtcNow.ToString(@"yyyy-MM-dd HH-mm-ss-fffffff") + ".log.resources");
        ...
        //{
        //    upgradeFileStreamWriter.WriteLine(FormatLogMessage(message));
        //    upgradeFileStreamWriter.Flush();
        //}

        private static string FormatLogMessage(string version, string message)
        {
            return DateTime.UtcNow.ToString(@"yyyy-MM-ddTHH\:mm\:ss") + " " + version + " - " + message;
        }

        private static void LogUpgradeStep(string version, string message, bool isImportant = true)
        {
            var niceLine = FormatLogMessage(version, message);
            DetailedLog += "\n" + niceLine;

            if (!isImportant && (isImportant || !SaveUnimportantDetails)) return;

            // sometimes a detail would be logged, when the file isn't open yet; then don't save
            if (!(upgradeFileStreamWriter?.BaseStream?.CanWrite ?? false)) return;

            upgradeFileStreamWriter.WriteLine(niceLine);
            upgradeFileStreamWriter.Flush();
            //LogUpgradeStep(version + " - " + message);
        }


        #region Version 5/6/7 Upgrade Methods and helpers
        /// <summary>
...
WHERE        (ToSIC_SexyContent_ContentGroupItems.SysDeleted IS NULL) AND (ModuleSettings.SettingName = N'ContentGroupID') AND 
        ...
        #region Version 8 Upgrade Methods
        ...
        private static void Version080303()
        {
        ...
        internal static string CheckUpgradeMessage(bool isSuperUser)
        {
            // Upgrade success check - show message if upgrade did not run successfully
            if (UpgradeComplete) return null;// || !UserInfo.IsSuperUser) return true;
